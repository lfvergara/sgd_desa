...
class Archivos {
	...
	function guardar_archivo() {
		if($this->archivo_id == 0){
			$sql = "INSERT INTO archivos (denominacion, matricula, tipo_id, tipo_documento, documento, ejercicio, activo_corriente, activo_nocorriente, activo, pasivo_corriente, 
              ...
              entidad, importe, excedente, numero_comprobante, cuenta_id, comentario, fecha, fecha_inicio, fecha_cierre, fecha_informe, adjunta_estadocontable, firma_digital, codigo_barras) 
							VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?, ?)";
			...
			$sql = "UPDATE archivos SET denominacion = ?, matricula = ?, tipo_id = ?, tipo_documento = ?, documento = ?, ejercicio = ? , activo_corriente = ?, activo_nocorriente = ?,
              ...
              comentario = ?, fecha_inicio = ?, fecha_cierre = ?, fecha_informe = ?, adjunta_estadocontable = ?, firma_digital = ?, codigo_barras = ? WHERE archivo_id = ?";
	...
	function actualizar_datos_observados_eecc() {
		$sql = "UPDATE archivos SET denominacion = ?, tipo_documento = ?, documento = ?, fecha_inicio = ?, fecha_cierre = ?, fecha_informe = ?, ejercicio = ?, activo_corriente = ?, activo_nocorriente = ?,
            activo = ?, pasivo_corriente = ?, pasivo_nocorriente = ?, pasivo = ?, patrimonio_neto = ?, bienes_uso = ?, venta_neta = ?, resultado_final = ?, comentario = ?, metodo_pago = ?, entidad = ?, numero_comprobante = ?, importe = ?, excedente = ? WHERE archivo_id = ?";
		$datos = array($this->denominacion, $this->tipo_documento, $this->documento, $this->fecha_inicio, $this->fecha_cierre, $this->fecha_informe, $this->ejercicio, $this->activo_corriente, 
                   $this->activo_nocorriente, $this->activo, $this->pasivo_corriente, $this->pasivo_nocorriente, $this->pasivo, $this->patrimonio_neto, $this->bienes_uso, $this->venta_neta,
                   $this->resultado_final, $this->comentario, $this->metodo_pago, $this->entidad, $this->numero_comprobante, $this->importe, $this->excedente, $this->archivo_id);
		execute_query($sql, $datos);
	}
